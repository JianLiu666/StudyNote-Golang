...
const (
	ROLE_BUYER  = iota // 買方
	ROLE_SELLER        // 賣方
	ROLE_END           // placeholder
)

func IsRoleTypeValid(n int) bool {
	return n >= 0 && n < ROLE_END
}
...
const (
	...
	ORDER_LIMIT         // 限價單
	ORDER_END           // placeholder
)

func IsOrderTypeValid(n int) bool {
	return n >= 0 && n < ORDER_END
}
...
const (
	...
	DURATION_END        // placeholder
)

func IsDurationTypeValid(n int) bool {
	return n >= 0 && n < DURATION_END
}
...
const (
	...
	STATUS_CANCEL         // 已取消
	STATUS_SUCCESS        // 已成交
	STATUS_END            // placeholder
)

func IsOrderStatusValid(n int) bool {
	return n >= 0 && n < STATUS_END
}
